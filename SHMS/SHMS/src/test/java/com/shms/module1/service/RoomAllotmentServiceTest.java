...
public class RoomAllotmentServiceTest {
    ...
    @Test
    void testRegisterAndAllotRoom_InvalidHall_ThrowsException() {
        // Arrange
        when(roomRepository.findByRoomType(RoomType.SINGLE)).thenReturn(List.of(singleRoom));
        when(hallRepository.findById("hallA")).thenReturn(Optional.empty()); // simulate missing hall
        ...
        assertEquals("Hall reference associated with room is invalid.", exception.getMessage());
        verify(userRepository, never()).save(any());
        verify(studentRepository, never()).save(any());
    }

    @Test
    void testRegisterAndAllotRoom_UsernameCollision_DerivesCleanly() {
        // Arrange
        when(roomRepository.findByRoomType(RoomType.SINGLE)).thenReturn(List.of(singleRoom));
        when(hallRepository.findById("hallA")).thenReturn(Optional.of(newHall));
        
        // Simulate "john.doe" exists, forcing fallback to "john.doe1"
        User existingUser = new User();
        when(userRepository.findByUsername(anyString()))
            .thenAnswer(i -> {
                String req = i.getArgument(0);
                if ("john.doe".equals(req)) {
        ...
        when(pdfGeneratorService.generateAllotmentLetter(any(), any(), any())).thenReturn(new byte[]{});

        // Act
        RegistrationResponse response = roomAllotmentService.registerAndAllotRoom(registrationRequest, adminUser);

        // Assert
        assertEquals("john.doe1", response.getGeneratedUsername());
        verify(userRepository, times(1)).save(argThat(user -> "john.doe1".equals(user.getUsername())));
    }
}
